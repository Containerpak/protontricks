...
RUN apt-get update && \
    ...
    curl --fail --location --output /tmp/protontricks-1.14.1-py3-none-any.whl \
    ...
    echo "${PROTONTRICKS_SHA256}  /tmp/protontricks-1.14.1-py3-none-any.whl" | sha256sum --check

FROM ghcr.io/containerpak/gtk:main

LABEL org.opencontainers.image.source="https://github.com/Containerpak/protontricks"

COPY --from=source /tmp/protontricks-1.14.1-py3-none-any.whl /tmp/protontricks-1.14.1-py3-none-any.whl
...
RUN apt-get update && \
    ...
    pip3 install --break-system-packages --no-deps /tmp/protontricks-1.14.1-py3-none-any.whl && \
    rm /tmp/protontricks-1.14.1-py3-none-any.whl && \
    cpak-clean-junk
